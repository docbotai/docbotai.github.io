...
if (window.marked && window.katex) {
    const mathExtension = {
        ...
        tokenizer(src) {
            ...
            if (match) {
                // Phân số, căn, số mũ... để trong một dòng rất dễ bị chồng trên màn hình hẹp.
                // Tự chuyển chúng thành công thức khối để KaTeX có đủ chiều cao.
                const hasTallMath = /\\(?:frac|dfrac|tfrac|sqrt|sum|prod|int|lim)|[_^]/.test(match[1]);
                return {
                    type: 'math',
                    raw: match[0],
                    text: match[1].trim(),
                    displayMode: match[0].startsWith('$$') || match[0].startsWith('\\[') || hasTallMath
                };
            }
        },
        renderer(token) {
...
function setAvatarContent(element, avatarUrl, fallbackText, altText) {
    if (!element) return;
...
function handleUserLogin(user) {
    ...
    const fallbackInitial = (firstName || email || 'U').charAt(0).toUpperCase();
    ...
    if (topAvatar) topAvatar.style.display = 'flex';
    setAvatarContent(userAvatar, avatarUrl, fallbackInitial, `Avatar ${fullName}`);
    setAvatarContent(topAvatar, avatarUrl, fallbackInitial, `Avatar ${fullName}`);
    if (modalEmail) modalEmail.textContent = email;
    if (modalGreeting) modalGreeting.textContent = `Chào ${firstName},`;
    setAvatarContent(accountModalAvatar, avatarUrl, fallbackInitial, `Avatar ${fullName}`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Resolve the signed-in user in the background, then replace history if needed.
    supabaseClient.auth.getUser()
        .then(({ data }) => {
            if (data?.user && currentUser?.id !== data.user.id) handleUserLogin(data.user);
        })
        .catch(() => {
            // Keep the already visible local history if the network is temporarily unavailable.
        });
});
...
function handleUserLogout() {
    ...
    document.getElementById('userAvatar')?.replaceChildren();
    const userName = document.getElementById('userName');
    const userEmail = document.getElementById('userEmail');
...
function activateSettingsItem(item, handler) {
    if (!item) return;
...
// These entries used to be decorative only. Activity now takes the user to the
// recent-chat list, while Personal Intelligence opens the current account panel.
activateSettingsItem(activityMenuItem, (event) => {
    event.stopPropagation();
    settingsDropdown?.classList.remove('active');
    const recentList = document.querySelector('.recent-list');
    recentList?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    recentList?.classList.add('activity-focus');
    window.setTimeout(() => recentList?.classList.remove('activity-focus'), 1600);
});

activateSettingsItem(personalIntelligenceMenuItem, (event) => {
    event.stopPropagation();
    settingsDropdown?.classList.remove('active');
    accountModal?.classList.add('active');
});
